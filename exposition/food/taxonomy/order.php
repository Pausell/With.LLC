...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Order</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdom</a>
<a href="phylum">Phylum</a>
<a href="class">Class</a>
<a href="order" class="subact"><strong>Order</strong></a>
...
$sql = "SELECT DISTINCT `order` FROM food ORDER BY `order` ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="orders" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['order']); // Split orders separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected order
        $url = 'orders?order=' . urlencode($need);

        // Output checkbox with anchor text for each order
        echo '<label>';
        echo '<input type="checkbox" name="order[]" value="' . htmlspecialchars($need) . '">';
