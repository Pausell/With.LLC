...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Kingdoms</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom" class="subact"><strong>Kingdoms</strong></a>
...
if (isset($_GET['kingdom'])) {
    if (is_array($_GET['kingdom'])) {
        $selectedItems = $_GET['kingdom'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"kingdoms?kingdom=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['kingdom'];
        echo "<p class=\"subact\"><a href=\"kingdom\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['kingdom'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all kingdoms from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['kingdom']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected kingdoms.";
        }
    } else {
        echo "No kingdoms found in the food database.";
    }
} else {
    echo "No kingdoms selected.";
}
