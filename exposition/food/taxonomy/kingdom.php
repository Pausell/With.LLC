...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Kingdom</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom" class="subact"><strong>Kingdom</strong></a>
...
$sql = "SELECT DISTINCT kingdom FROM food ORDER BY kingdom ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="kingdoms" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['kingdom']); // Split kingdoms separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected kingdom
        $url = 'kingdoms?kingdom=' . urlencode($need);

        // Output checkbox with anchor text for each kingdom
        echo '<label>';
        echo '<input type="checkbox" name="kingdom[]" value="' . htmlspecialchars($need) . '">';
