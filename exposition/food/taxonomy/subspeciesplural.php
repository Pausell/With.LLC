...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Subspecies (plural)</h1></span>
...
<a href="subspecies" class="subact"><strong>Subspecies (plural)</strong></a>
...
<?php
if (isset($_GET['subspecies'])) {
    if (is_array($_GET['subspecies'])) {
        $selectedItems = $_GET['subspecies'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"subspeciesplural?subspecies=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['subspecies'];
        echo "<p class=\"subact\"><a href=\"subspecies\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['subspecies'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all subspeciesplural from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['subspecies']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected subspeciesplural.";
        }
    } else {
        echo "No subspecies (plural) found in the food database.";
    }
} else {
    echo "No subspecies (plural) selected.";
}
