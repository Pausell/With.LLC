...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Forms</h1></span>
...
<a href="form" class="subact"><strong>Forms</strong></a>
...
if (isset($_GET['form'])) {
    if (is_array($_GET['form'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"forms?form=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['form'];
        echo "<p class=\"subact\"><a href=\"form\">" . $selectedItem . "</a></p>";
    }
}
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all forms from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['form']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected forms.";
        }
    } else {
        echo "No forms found in the food database.";
    }
} else {
