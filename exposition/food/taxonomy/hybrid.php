...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Hybrid</h1></span>
...
<a href="hybrid" class="subact"><strong>Hybrid</strong></a>
<a href="form">Form</a>
...
$sql = "SELECT DISTINCT hybrid FROM food ORDER BY hybrid ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="hybrids" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['hybrid']); // Split hybrids separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected hybrid
        $url = 'hybrids?hybrid=' . urlencode($need);

        // Output checkbox with anchor text for each hybrid
        echo '<label>';
        echo '<input type="checkbox" name="hybrid[]" value="' . htmlspecialchars($need) . '">';
