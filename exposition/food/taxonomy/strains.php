...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Strains</h1></span>
...
<a href="strain" class="subact"><strong>Strains</strong></a>
...
if (isset($_GET['strain'])) {
    if (is_array($_GET['strain'])) {
        $selectedItems = $_GET['strain'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"strains?strain=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['strain'];
        echo "<p class=\"subact\"><a href=\"strain\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['strain'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all strains from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['strain']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected strains.";
        }
    } else {
        echo "No strains found in the food database.";
    }
} else {
    echo "No strains selected.";
}
