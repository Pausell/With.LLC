...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Clade</h1></span>
...
<a href="clade" class="subact"><strong>Clade</strong></a>
...
$sql = "SELECT DISTINCT clade FROM food ORDER BY clade ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="clades" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['clade']); // Split clades separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected clade
        $url = 'clades?clade=' . urlencode($need);

        // Output checkbox with anchor text for each clade
        echo '<label>';
        echo '<input type="checkbox" name="clade[]" value="' . htmlspecialchars($need) . '">';
