...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Classes</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdoms</a>
<a href="phylum">Phylum</a>
<a href="class" class="subact"><strong>Classes</strong></a>
...
if (isset($_GET['class'])) {
    if (is_array($_GET['class'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"classes?class=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['class'];
        echo "<p class=\"subact\"><a href=\"class\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['class'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all classes from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['class']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected classes.";
        }
    } else {
        echo "No classes found in the food database.";
    }
} else {
    echo "No classes selected.";
}
