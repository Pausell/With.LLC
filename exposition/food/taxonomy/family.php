...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Family</h1></span>
...
<a href="family" class="subact"><strong>Family</strong></a>
...
$sql = "SELECT DISTINCT family FROM food ORDER BY family ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="families" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['family']); // Split families separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected family
        $url = 'families?family=' . urlencode($need);

        // Output checkbox with anchor text for each family
        echo '<label>';
        echo '<input type="checkbox" name="family[]" value="' . htmlspecialchars($need) . '">';
