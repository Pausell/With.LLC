...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Class</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdom</a>
<a href="phylum">Phylum</a>
<a href="class" class="subact"><strong>Class</strong></a>
...
$sql = "SELECT DISTINCT class FROM food ORDER BY class ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="classes" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['class']); // Split classes separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected class
        $url = 'classes?class=' . urlencode($need);

        // Output checkbox with anchor text for each class
        echo '<label>';
        echo '<input type="checkbox" name="class[]" value="' . htmlspecialchars($need) . '">';
