...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Genera</h1></span>
...
<a href="genus" class="subact"><strong>Genera</strong></a>
...
<?php
if (isset($_GET['genud'])) {
    if (is_array($_GET['genud'])) {
        $selectedItems = $_GET['genud'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"genera?genud=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['genud'];
        echo "<p class=\"subact\"><a href=\"genud\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['genud'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all genera from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['genud']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected genera.";
        }
    } else {
        echo "No genera found in the food database.";
    }
} else {
    echo "No genera selected.";
}
