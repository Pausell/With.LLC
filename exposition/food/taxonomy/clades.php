...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Clades</h1></span>
...
<a href="clade" class="subact"><strong>Clades</strong></a>
...
<?php
if (isset($_GET['clade'])) {
    if (is_array($_GET['clade'])) {
        $selectedItems = $_GET['clade'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"clades?clade=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['clade'];
        echo "<p class=\"subact\"><a href=\"clade\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['clade'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all clades from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['clade']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected clades.";
        }
    } else {
        echo "No clades found in the food database.";
    }
} else {
    echo "No clades selected.";
}
