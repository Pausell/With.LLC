...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Form</h1></span>
...
<a href="form" class="subact"><strong>Form</strong></a>
...
$sql = "SELECT DISTINCT form FROM food ORDER BY form ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="forms" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['form']); // Split forms separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected form
        $url = 'forms?form=' . urlencode($need);

        // Output checkbox with anchor text for each form
        echo '<label>';
        echo '<input type="checkbox" name="form[]" value="' . htmlspecialchars($need) . '">';
