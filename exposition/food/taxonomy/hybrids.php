...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Hybrids</h1></span>
...
<a href="hybrid" class="subact"><strong>Hybrids</strong></a>
<a href="form">Form</a>
...
<?php
if (isset($_GET['hybrid'])) {
    if (is_array($_GET['hybrid'])) {
        $selectedItems = $_GET['hybrid'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"hybrids?hybrid=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['hybrid'];
        echo "<p class=\"subact\"><a href=\"hybrid\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['hybrid'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all hybrids from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['hybrid']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected hybrids.";
        }
    } else {
        echo "No hybrids found in the food database.";
    }
} else {
    echo "No hybrids selected.";
}
