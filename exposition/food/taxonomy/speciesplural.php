...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Species (plural)</h1></span>
...
<a href="species" class="subact"><strong>Species (plural)</strong></a>
...
if (isset($_GET['species'])) {
    if (is_array($_GET['species'])) {
        $selectedItems = $_GET['species'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"speciesplural?species=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['species'];
        echo "<p class=\"subact\"><a href=\"species\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['species'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all speciesplural from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['species']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected speciesplural.";
        }
    } else {
        echo "No species (plural) found in the food database.";
    }
} else {
    echo "No species (plural) selected.";
}
