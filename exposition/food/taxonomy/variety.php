...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Variety</h1></span>
...
<a href="variety"><strong>Variety</strong></a>
...
$sql = "SELECT DISTINCT variety FROM food ORDER BY variety ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="varieties" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['variety']); // Split varieties separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected variety
        $url = 'varieties?variety=' . urlencode($need);

        // Output checkbox with anchor text for each variety
        echo '<label>';
        echo '<input type="checkbox" name="variety[]" value="' . htmlspecialchars($need) . '">';
