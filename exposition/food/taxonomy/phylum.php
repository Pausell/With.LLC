...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Phylum</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdom</a>
<a href="phylum" class="subact"><strong>Phylum</strong></a>
...
$sql = "SELECT DISTINCT phylum FROM food ORDER BY phylum ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="phyla" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['phylum']); // Split phyla separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected phylum
        $url = 'phyla?phylum=' . urlencode($need);

        // Output checkbox with anchor text for each phylum
        echo '<label>';
        echo '<input type="checkbox" name="phylum[]" value="' . htmlspecialchars($need) . '">';
