...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Genus</h1></span>
...
<a href="genus" class="subact"><strong>Genus</strong></a>
...
$sql = "SELECT DISTINCT genus FROM food ORDER BY genus ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="genera" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['genus']); // Split genera separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected genus
        $url = 'genera?genus=' . urlencode($need);

        // Output checkbox with anchor text for each genus
        echo '<label>';
        echo '<input type="checkbox" name="genus[]" value="' . htmlspecialchars($need) . '">';
