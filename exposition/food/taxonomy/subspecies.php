...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Subspecies</h1></span>
...
<a href="subspecies" class="subact"><strong>Subspecies</strong></a>
...
$sql = "SELECT DISTINCT subspecies FROM food ORDER BY subspecies ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="subspeciesplural" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['subspecies']); // Split subspeciesplural separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected subspecies
        $url = 'subspeciesplural?subspecies=' . urlencode($need);

        // Output checkbox with anchor text for each subspecies
        echo '<label>';
        echo '<input type="checkbox" name="subspecies[]" value="' . htmlspecialchars($need) . '">';
    ...
    echo "No subspecies (plural) found.";
}
