...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Varieties</h1></span>
...
<a href="variety"><strong>Varieties</strong></a>
...
if (isset($_GET['variety'])) {
    if (is_array($_GET['variety'])) {
        $selectedItems = $_GET['variety'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"varieties?variety=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['variety'];
        echo "<p class=\"subact\"><a href=\"variety\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['variety'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all varieties from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['variety']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected varieties.";
        }
    } else {
        echo "No varieties found in the food database.";
    }
} else {
    echo "No varieties selected.";
}
