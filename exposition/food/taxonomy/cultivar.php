...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Cultivar</h1></span>
...
<a href="form">Form</a>
<a href="cultivar" class="subact"><strong>Cultivar</strong></a>
</div>
</div>
...
$sql = "SELECT DISTINCT cultivar FROM food ORDER BY cultivar ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="cultivars" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['cultivar']); // Split cultivars separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected cultivar
        $url = 'cultivars?cultivar=' . urlencode($need);

        // Output checkbox with anchor text for each cultivar
        echo '<label>';
        echo '<input type="checkbox" name="cultivar[]" value="' . htmlspecialchars($need) . '">';
