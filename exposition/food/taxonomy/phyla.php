...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Phyla</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdom</a>
<a href="phylum" class="subact"><strong>Phyla</strong></a>
...
if (isset($_GET['phylum'])) {
    if (is_array($_GET['phylum'])) {
        $selectedItems = $_GET['phylum'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"phyla?phylum=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['phylum'];
        echo "<p class=\"subact\"><a href=\"phylum\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['phylum'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all phyla from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['phylum']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected phyla.";
        }
    } else {
        echo "No phyla found in the food database.";
    }
} else {
    echo "No phyla selected.";
}
