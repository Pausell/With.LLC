...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Orders</h1></span>
<?php include 'indexnav.php'; ?>
<div class="crumbs">
<a href="kingdom">Kingdom</a>
<a href="phylum">Phylum</a>
<a href="class">Class</a>
<a href="order" class="subact"><strong>Orders</strong></a>
...
if (isset($_GET['order'])) {
    if (is_array($_GET['order'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"orders?order=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['order'];
        echo "<p class=\"subact\"><a href=\"order\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['order'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all orders from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['order']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected orders.";
        }
    } else {
        echo "No orders found in the food database.";
    }
} else {
