...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Strain</h1></span>
...
<a href="strain" class="subact"><strong>Strain</strong></a>
...
$sql = "SELECT DISTINCT strain FROM food ORDER BY strain ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="strains" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['strain']); // Split strains separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected strain
        $url = 'strains?strain=' . urlencode($need);

        // Output checkbox with anchor text for each strain
        echo '<label>';
        echo '<input type="checkbox" name="strain[]" value="' . htmlspecialchars($need) . '">';
