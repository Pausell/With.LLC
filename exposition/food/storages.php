...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Storages</h1></span>
...
<a href="storage" class="subact"><strong>Storages</strong></a>
...
if (isset($_GET['storage'])) {
    if (is_array($_GET['storage'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"storages?storage=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['storage'];
        echo "<p class=\"subact\"><a href=\"storage\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['storage'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all storages from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['storage']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected storages.";
        }
    } else {
        echo "No storages found in the food database.";
    }
} else {
    echo "No storages selected.";
}
