...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Harvests</h1></span>
...
<a href="harvesting" class="subact"><strong>Harvests</strong></a>
...
<?php
if (isset($_GET['harvesting'])) {
    if (is_array($_GET['harvesting'])) {
        $selectedItems = $_GET['harvesting'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"harvests?harvesting=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['harvesting'];
        echo "<p class=\"subact\"><a href=\"harvesting\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['harvesting'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all harvests from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['harvesting']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected harvests.";
        }
    } else {
        echo "No harvests found in the food database.";
    }
} else {
    echo "No harvests selected.";
}
