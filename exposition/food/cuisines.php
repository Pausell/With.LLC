...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Cuisines</h1></span>
...
<a href="cuisine" class="subact"><strong>Cuisines</strong></a>
...
if (isset($_GET['cuisine'])) {
    if (is_array($_GET['cuisine'])) {
        $selectedItems = $_GET['cuisine'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"cuisines?cuisine=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['cuisine'];
        echo "<p class=\"subact\"><a href=\"cuisine\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['cuisine'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all cuisines from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['cuisine']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected cuisines.";
        }
    } else {
        echo "No cuisines found in the food database.";
    }
} else {
    echo "No cuisines selected.";
}
