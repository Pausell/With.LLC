...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">History</h1></span>
...
<a href="history" class="subact"><strong>History</strong></a>
<a href="use">Use</a>
</div>
</div>
...
$sql = "SELECT DISTINCT history FROM food ORDER BY history ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="histories" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['history']); // Split histories separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected history
        $url = 'histories?history=' . urlencode($need);

        // Output checkbox with anchor text for each history
        echo '<label>';
        echo '<input type="checkbox" name="history[]" value="' . htmlspecialchars($need) . '">';
