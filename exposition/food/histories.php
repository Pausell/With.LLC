...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Histories</h1></span>
...
<a href="history" class="subact"><strong>Histories</strong></a>
<a href="use">Use</a>
</div>
</div>
...
if (isset($_GET['history'])) {
    if (is_array($_GET['history'])) {
        $selectedItems = $_GET['history'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"histories?history=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['history'];
        echo "<p class=\"subact\"><a href=\"history\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['history'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all histories from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['history']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected histories.";
        }
    } else {
        echo "No histories found in the food database.";
    }
} else {
    echo "No histories selected.";
}
