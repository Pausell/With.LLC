...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Nutrition (plural)</h1></span>
...
<a href="nutrition" class="subact"><strong>Nutrition (plural)</strong></a>
...
if (isset($_GET['nutrition'])) {
    if (is_array($_GET['nutrition'])) {
        $selectedItems = $_GET['nutrition'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"nutritionplural?nutrition=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['nutrition'];
        echo "<p class=\"subact\"><a href=\"nutrition\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['nutrition'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all nutritionplural from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['nutrition']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected nutrition (plural).";
        }
    } else {
        echo "No nutrition (plural) found in the food database.";
    }
} else {
    echo "No nutrition (plural) selected.";
}
