...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Habitat</h1></span>
...
<a href="habitat" class="subact"><strong>Habitat</strong></a>
...
$sql = "SELECT DISTINCT habitat FROM food ORDER BY habitat ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="habitats" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['habitat']); // Split habitats separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected habitat
        $url = 'habitats?habitat=' . urlencode($need);

        // Output checkbox with anchor text for each habitat
        echo '<label>';
        echo '<input type="checkbox" name="habitat[]" value="' . htmlspecialchars($need) . '">';
