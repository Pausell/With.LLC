...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Seasons</h1></span>
...
<a href="season" class="subact"><strong>Seasons</strong></a>
...
if (isset($_GET['season'])) {
    if (is_array($_GET['season'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"seasons?season=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['season'];
        echo "<p class=\"subact\"><a href=\"season\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['season'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all seasons from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['season']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected seasons.";
        }
    } else {
        echo "No seasons found in the food database.";
    }
} else {
    echo "No seasons selected.";
}
