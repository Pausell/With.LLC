...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Harvesting</h1></span>
...
<a href="harvesting" class="subact"><strong>Harvesting</strong></a>
...
$sql = "SELECT DISTINCT harvesting FROM food ORDER BY harvesting ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="harvests" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['harvesting']); // Split harvests separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected harvesting
        $url = 'harvests?harvesting=' . urlencode($need);

        // Output checkbox with anchor text for each harvesting
        echo '<label>';
        echo '<input type="checkbox" name="harvesting[]" value="' . htmlspecialchars($need) . '">';
