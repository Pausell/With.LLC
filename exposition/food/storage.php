...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Storage</h1></span>
...
<a href="storage" class="subact"><strong>Storage</strong></a>
...
$sql = "SELECT DISTINCT storage FROM food ORDER BY storage ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="storages" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['storage']); // Split storages separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected storage
        $url = 'storages?storage=' . urlencode($need);

        // Output checkbox with anchor text for each storage
        echo '<label>';
        echo '<input type="checkbox" name="storage[]" value="' . htmlspecialchars($need) . '">';
