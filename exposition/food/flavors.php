...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Flavors</h1></span>
...
<a href="flavor" class="subact"><strong>Flavors</strong></a>
...
if (isset($_GET['flavor'])) {
    if (is_array($_GET['flavor'])) {
        $selectedItems = $_GET['flavor'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"flavors?flavor=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['flavor'];
        echo "<p class=\"subact\"><a href=\"flavor\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['flavor'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all flavors from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['flavor']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected flavors.";
        }
    } else {
        echo "No flavors found in the food database.";
    }
} else {
    echo "No flavors selected.";
}
