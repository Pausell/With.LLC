...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Use</h1></span>
...
$sql = "SELECT DISTINCT `use` FROM food ORDER BY `use` ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="uses" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['use']); // Split uses separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected use
        $url = 'uses?use=' . urlencode($need);

        // Output checkbox with anchor text for each use
        echo '<label>';
        echo '<input type="checkbox" name="use[]" value="' . htmlspecialchars($need) . '">';
