...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Season</h1></span>
...
<a href="season" class="subact"><strong>Season</strong></a>
...
$sql = "SELECT DISTINCT season FROM food ORDER BY season ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="seasons" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['season']); // Split seasons separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected season
        $url = 'seasons?season=' . urlencode($need);

        // Output checkbox with anchor text for each season
        echo '<label>';
        echo '<input type="checkbox" name="season[]" value="' . htmlspecialchars($need) . '">';
