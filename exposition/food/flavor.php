...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Flavor</h1></span>
...
<a href="flavor" class="subact"><strong>Flavor</strong></a>
...
$sql = "SELECT DISTINCT flavor FROM food ORDER BY flavor ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="flavors" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['flavor']); // Split flavors separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected flavor
        $url = 'flavors?flavor=' . urlencode($need);

        // Output checkbox with anchor text for each flavor
        echo '<label>';
        echo '<input type="checkbox" name="flavor[]" value="' . htmlspecialchars($need) . '">';
