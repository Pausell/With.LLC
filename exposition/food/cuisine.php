...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Cuisine</h1></span>
...
<a href="cuisine" class="subact"><strong>Cuisine</strong></a>
...
$sql = "SELECT DISTINCT cuisine FROM food ORDER BY cuisine ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="cuisines" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['cuisine']); // Split cuisines separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected cuisine
        $url = 'cuisines?cuisine=' . urlencode($need);

        // Output checkbox with anchor text for each cuisine
        echo '<label>';
        echo '<input type="checkbox" name="cuisine[]" value="' . htmlspecialchars($need) . '">';
