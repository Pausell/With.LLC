...
<span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Uses</h1></span>
...
<a href="use" class="subact"><strong>Use</strong></a>
</div>
</div>
...
<?php
if (isset($_GET['use'])) {
    if (is_array($_GET['use'])) {
        $selectedItems = $_GET['use'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"uses?use=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['use'];
        echo "<p class=\"subact\"><a href=\"use\">" . $selectedItem . "</a></p>";
    }
}
...
if (!empty($selectedNeeds)) {
    ...
    // Fetch all uses from the 'food' table
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['use']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No food found for the selected uses.";
        }
    } else {
        echo "No uses found in the food database.";
    }
} else {
    echo "No uses selected.";
}
