  ...
  <div id="container">
   ...
   <div style="position:relative">
    <span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Vocal Expanse Network Tool <em style="opacity:.8">Using</em> Real Elements</h1></span>
     <div class="crumbs">
      <a href="playlist" class="subact"><strong>Playlists</strong></a>
...
if (isset($_GET['playlist'])) {
    if (is_array($_GET['playlist'])) {
        $selectedItems = $_GET['playlist'];
        echo "<p class=\"subact\">";
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"playlists?playlist=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['playlist'];
        echo "<p class=\"subact\"><a href=\"playlist\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['playlist'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['playlist']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No songs found for the selected playlists.";
        }
    } else {
        echo "No songs found in the music database.";
    }
} else {
    echo "No playlists selected.";
}
