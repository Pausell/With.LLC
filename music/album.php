  ...
  <div id="container">
   ...
   <div style="position:relative">
    ...
    <div class="crumbs">
     ...
     <a href="/music" class="subact"><strong>Album</strong></a>
     <a href="song">Song</a>
...
$sql = "SELECT DISTINCT album FROM music ORDER BY album ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="albums" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['album']); // Split albums separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected album
        $url = 'albums?album=' . urlencode($need);

        // Output checkbox with anchor text for each album
        echo '<label>';
        echo '<input type="checkbox" name="album[]" value="' . htmlspecialchars($need) . '">';
