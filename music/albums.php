  ...
  <div id="container">
   ...
   <div style="position:relative">
    ...
    <div class="crumbs">
     ...
     <a href="album" class="subact"><strong>Albums</strong></a>
     <a href="song">Song</a>
...
if (isset($_GET['album'])) {
    if (is_array($_GET['album'])) {
        ...
        foreach ($selectedItems as $selectedItem) {
            echo "<a href=\"albums?album=" . urlencode($selectedItem) . "\">" . $selectedItem . "</a>";
            $counter++;
            if ($counter < $numItems) {
                echo ", ";
            }
        ...
        $selectedItem = $_GET['album'];
        echo "<p class=\"subact\"><a href=\"album\">" . $selectedItem . "</a></p>";
    }
}
...
$selectedNeeds = $_GET['album'] ?? array();
...
if (!empty($selectedNeeds)) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $needNames = explode(', ', $row['album']); // Split needs separated by commas
        ...
        if (!$needsFound) {
            echo "No songs found for the selected albums.";
        }
    } else {
        echo "No songs found in the music database.";
    }
} else {
    echo "No albums selected.";
}
