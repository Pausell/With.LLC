  ...
  <div id="container">
   ...
   <div style="position:relative">
    <span style="position:absolute;opacity:.1;z-index:-1"><h1 style="margin:0">Vocal Expanse Network Tool <em style="opacity:.8">Using</em> Real Elements</h1></span>
     <div class="crumbs">
     <a href="/music" class="subact"><strong>Playlist</strong></a>
...
$sql = "SELECT DISTINCT playlist FROM music ORDER BY playlist ASC";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    echo '<form action="playlists" method="get" class="bulklist">';
    ...
    while ($row = $result->fetch_assoc()) {
        $needs = explode(', ', $row['playlist']); // Split playlists separated by commas
    ...
    foreach ($processedNeeds as $need) {
        // Generate the URL to view songs for the selected playlist
        $url = 'playlists?playlist=' . urlencode($need);

        // Output checkbox with anchor text for each playlist
        echo '<label>';
        echo '<input type="checkbox" name="playlist[]" value="' . htmlspecialchars($need) . '">';
